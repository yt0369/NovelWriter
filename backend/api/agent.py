...
async def agent_ws(ws: WebSocket, project_id: str, session_id: str):
    ...
    pending_changes: dict[str, PendingChange] = {}
            ...
            if msg_type == "chat":
                ...
                try:
                    ...
                    if isinstance(parsed, dict) and parsed.get("type") == "questionnaire_answer":
                        from core.tools.questionnaire_tools import answer_questionnaire, clear_questionnaire
                        answers = parsed.get("answers", {})
                        q_result = answer_questionnaire(project_id, answers)
                        clear_questionnaire(project_id)
                        # 将答案格式化为用户消息
                        answer_lines = []
                ...
                async for event in run_agent(chat_messages, provider, project_id, project_dir, active_file_path=active_file_path, turn_id=turn_id, session_id=session_id):
                    ...
                    if etype == "delta":
                        ...
                        pc = event.get("pending_change", {})
                        pending_changes[pc["id"]] = PendingChange(
                            id=pc["id"],
                ...
                pc = pending_changes.get(change_id)
                try:
...
async def approve_change(project_id: str, change_id: str):
    print(f"[DEBUG] approve_change: project={project_id}, change={change_id}", flush=True)
    try:
        from core.pending_changes import get_pending_change, resolve_project_file, check_ai_write_allowed
        import shutil
        change = await get_pending_change(project_id, change_id)
        if not change:
            raise HTTPException(status_code=404, detail=f"未找到待审批变更: {change_id}")
        if change["status"] != "pending":
            raise HTTPException(status_code=400, detail=f"变更状态为 {change['status']}，不能批准")
        ...
        if tool_name == "delete_file":
            ...
            # 重命名操作：new_content 存储的是新名称
            new_name = change["new_content"]
            new_path = file_path.parent / new_name
            file_path.rename(new_path)
            rel_path = str(new_path.relative_to(file_path.parent.parent)).replace("\\", "/")
        else:
            # 写入操作
        ...
        print(f"[DEBUG] approve_change success: {rel_path}", flush=True)
        return {"id": change_id, "file_path": rel_path, "status": "approved"}
    except HTTPException:
        raise
    except Exception as e:
        print(f"[DEBUG] approve_change error: {e}", flush=True)
...
async def get_messages(project_id: str, session_id: str):
    return await _load_chat_messages(project_id, session_id)
